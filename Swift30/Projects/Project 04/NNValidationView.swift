...
class NNValidationView: UIView {
    ...
    func randomColor() -> UIColor {
        return UIColor.init(_colorLiteralRed: (Float)(arc4random() % 100) / 100.0, green: (Float)(arc4random() % 100) / 100.0, blue: (Float)(arc4random() % 100) / 100.0, alpha: 0.5)
    }
...
//        let charSize = textString.substring(to: textString.startIndex).size(withAttributes: [NSAttributedString.Key.font : UIFont.systemFont(ofSize: 14)])
        let charSize = String(textString[..<textString.startIndex]).size(withAttributes: [NSAttributedString.Key.font : UIFont.systemFont(ofSize: 14)])
        ...
        var pointX: CGFloat
        var pointY: CGFloat
        for i in 0..<textString.count {
            let char = CGFloat(i)
            pointX = (CGFloat)(arc4random() % UInt32(Float(width))) + rect.size.width / (CGFloat)(textString.count) * char
            ...
            string.draw(at: point,withAttributes:([NSAttributedString.Key.font : UIFont.systemFont(ofSize: 14)]))
        }
        drawLine(rect)
    }
